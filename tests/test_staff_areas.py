...
def test_staff_can_add_area(client):
    ...
    login(client, staff)
    client.post("/staff/areas/add", data={"name": "Graph theory"})
    assert AreaOfInterest.query.filter_by(staff_id=staff.id).count() == 1


def test_add_area_rejects_blank_name(client):
    staff = create_user(role=ROLE_STAFF, email="s2@example.com")
    login(client, staff)
    client.post("/staff/areas/add", data={"name": "   "})
    assert AreaOfInterest.query.count() == 0


def test_add_area_rejects_case_insensitive_duplicate(client):
    staff = create_user(role=ROLE_STAFF, email="s3@example.com")
    login(client, staff)
    client.post("/staff/areas/add", data={"name": "Data Analysis"})
    client.post("/staff/areas/add", data={"name": "data analysis"})
    assert AreaOfInterest.query.count() == 1


def test_staff_can_edit_and_delete_area(client):
    staff = create_user(role=ROLE_STAFF, email="s4@example.com")
    login(client, staff)
    client.post("/staff/areas/add", data={"name": "Algorthms"})  # typo on purpose
    area = AreaOfInterest.query.first()
    client.post(f"/staff/areas/{area.id}/edit", data={"name": "Algorithms"})
    from app.extensions import db
    assert db.session.get(AreaOfInterest, area.id).name == "Algorithms"
    client.post(f"/staff/areas/{area.id}/delete")
    assert AreaOfInterest.query.count() == 0


def test_staff_cannot_edit_another_staffs_area(client):
    owner = create_user(role=ROLE_STAFF, email="owner@example.com")
    other = create_user(role=ROLE_STAFF, email="other@example.com")
    login(client, owner)
    client.post("/staff/areas/add", data={"name": "Security"})
    area = AreaOfInterest.query.first()
